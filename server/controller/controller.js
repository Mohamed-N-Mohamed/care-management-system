...
exports.update = (req, res) => {
  ...
  .then(data => {
    if(!data){
      res.status(400).send({ message: "Cannot Update user with the same ID or client does not exist " });
    } else {
      res.send(data)
    }
  })
...
exports.delete = (req, res) => {
  ...
  .then(data => {
    if(!data){
      res.status(404).send({message: 'Cannot delete client, check if the id is right'})
    }
    else {
      res.send({
